        ...
        <div className="flex items-start justify-between border-b border-slate-100 pb-3">
          <div>
            <span className="text-[10px] uppercase font-bold text-emerald-800 bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-200">
              {t('auth.doctorAccessPill', 'Vaidya Practitioner Portal')}
            </span>
            <h2 className="text-xl font-extrabold text-slate-900 mt-1.5 flex items-center gap-2">
              <Stethoscope className="w-5 h-5 text-emerald-700" />
              <span>{t('auth.standaloneDoctorTitle', 'Vaidya Console OTP Sign In')}</span>
            </h2>
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              {t('auth.standaloneDoctorSub', 'Authenticate with Doctor Registration ID or Mobile Number.')}
            </p>
          </div>
          <button
            type="button"
